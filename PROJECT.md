...
| iOS lock screen / Control Center only shows track metadata when audio is playing in the active browser tab AND we tell the OS via the Media Session API | `setupMediaSession()` registers play/pause/next/prev/seek handlers once; `updateMediaSession()` sets the title/artist/album + position from each `fetchState`. We deliberately omit `artwork` so the lock screen falls back to the CleanPlay favicon (text-only by design). Metadata only appears when CleanPlay is the active playback device (▶ Here) |
